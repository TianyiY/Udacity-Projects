...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            if 0<=self.epsilon and self.epsilon<=1:
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if random.random()<self.epsilon:
                ...
                for key, value in self.Q[state].items():
                    if value==maxQ:
                        maxAction.append(key)
                action=random.choice(maxAction)
                if action=='None':
                    action=None
        return action
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            if state in self.Q.keys() and str(action) in self.Q[state].keys():
                self.Q[state][str(action)] = (1-self.alpha) * self.Q[state][str(action)] + self.alpha * reward
        return
